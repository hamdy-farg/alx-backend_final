...
class BaseModel(db.Model):
    ...
    def check_image(self, request_data=None, file=None):
        ...
        if file is not None:
            ...
            if file_extenstion not in IMAGE_EXTENTIONS:
                ...
                return (
                    "this file extenstion is not allowed please send [jpg, jpeg, png]"
                )
            return file
        else:
    ...
    def save_image(self, folder_name: str, request_data=None, file=None):
        ...
        try:
            ...
            image_io = io.BytesIO()
            image.save(image_io, "JPEG", quality=30)
            image_io.seek(0)
            print("arrived2")
    ...
    def convert_image_to_link(self, route: str, image_id: str):
        ...
        image_link = (
            f"{os.getenv('LOCALHOST','http://127.0.0.1:5000')}"
            + f"{route}"
            + f"{image_id}"
        )
        return image_link
    ...
    def validate_user_data(self, user_data, request):
        ...
        if error_msg is None and user_data.get("email_address") is not None:
            ...
            filtered_user = self.__class__.query.filter(
                self.__class__.email_address == user_data.get("email_address")
            ).first()
    ...
    def delete_image(self, image_name: str, image_folder: str):
        # Define the folder where images are stored

        try:
            folder_path = os.path.join(os.getcwd(), "assets", "user", image_folder)
