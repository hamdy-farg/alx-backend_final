...
class PlainUserRegisterSchema(PlainUserLoginSchema):
    ...
    secret = fields.Str()
    image = fields.Str(dump_only=True)
    is_confirmed = fields.Str(dump_only=True)
...
class SuccessSchema(Schema):
    ...
    status = fields.Bool(required=True, description="Indicates the success status")
...
class ClientBookListSchema(Schema):
    Bookings = fields.List(fields.Nested(PlainBookingsSchema), dump_only=True)
...
# booked = fields.List(fields.Nested(PlainBookedSchema),dump_only=True)
class PlainNotificationSubscribeSchema(Schema):
    fcm_token = fields.String(required=True)
    role = fields.Enum(enum=RoleEnum, required=True)
...
class NotificationListSchema(Schema):
    notificatoins = fields.List(fields.Nested(PlainNotificationSchema), dump_only=True)


# class ClientBookQueryParamsSchema(Schema):
#     title
