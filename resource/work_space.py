...
from schema import (
    PlainGetWorkSpace,
    PlainUpdateWorkSpaceSchema,
    PlainWorkSpaceSchema,
    SuccessSchema,
    WorkSpaceSchema,
)
...
class WorkSpcae(MethodView):
    ...
    def post(self, work_space_data):
        ...
        workspaceTitle = WorkSpaceModel.query.filter(
            WorkSpaceModel.title == work_space_data.get("title")
        ).first()
        if workspaceTitle is not None:
            abort(
                400, message="this work space title is taken before choose another one"
            )
        ...
        work_space_image_saved = work_space.save_image(
            request_data=request, folder_name="work_space_pics"
        )
        ...
        if work_space_saved:
            ...
            work_space.image = work_space.convert_image_to_link(
                route="/workspace/image/", image_id=work_space.id
            )
            return work_space
        else:
    ...
    def put(self, work_space_data):
        ...
        work_space = WorkSpaceModel.query.filter(
            WorkSpaceModel.id == work_space_data.get("work_space_id")
        ).first()
        print(work_space)
        ...
        try:
            if request.files["image"] is not None:
                print("entered2")
                work_space_saved = work_space.save_image(
                    folder_name="work_space_pics", request_data=request
                )
    ...
    def get(self, work_space_data):
        ...
        work_space = WorkSpaceModel.query.filter(
            WorkSpaceModel.id == work_space_id
        ).first()
        work_space.image = work_space.convert_image_to_link(
            route="/workspace/image/", image_id=work_space.id
        )
    ...
    def delete(self, work_space_data):
        ...
        try:
            ...
            work_space.delete_image(
                image_folder="work_space_pics", image_name=f"{work_space.id}"
            )

            work_space_deleted = work_space.delete()
            if work_space_deleted:
                return {"code": 200, "message": "deleted successfully", "status": True}
            else:
    ...
    def get(self, work_space_id):
        ...
        if work_space is not None:
            ...
            imageName = os.path.join(
                os.getcwd(), "assets", "user", "work_space_pics", work_space.image
            )
...
class UserWorkSpaces(MethodView):
    ...
    def get(self):
        ...
        admin = UserModel.query.filter(
            UserModel.id == user_id, UserModel.role == RoleEnum.admin
        ).first()
        #
        work_spaces = None
        try:
