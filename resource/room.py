...
class Room(MethodView):
    ...
    def post(self, room_data):
        room_exists = RoomModel.query.filter(
            RoomModel.title == room_data.get("title")
        ).first()
        print(room_exists)
        ...
        #
        room_image_saved = room.save_image(
            request_data=request, folder_name="room_pics"
        )
        ...
        if room_is_save:
            room.image = room.convert_image_to_link(
    ...
    @blp.arguments(RoomSchema, location="query")
    ...
    def delete(self, room_data):
        ...
        if room is not None:
            ...
            if room_deleted:
                return {"code": 200, "message": "deleted", "status": True}
            else:
...
class GetAllWorkSpaceRoom(MethodView):
    ...
    def post(self, room_data):
        """to get work space rooms"""
        workSpace = WorkSpaceModel.query.filter(
            WorkSpaceModel.id == room_data.get("work_space_id")
        ).first()
        RoomsList = []
        if workSpace is not None:
            ...
            for workSpaceRoom in workSpaceRoomsList:
                workSpaceRoom.image = workSpaceRoom.convert_image_to_link(
                    route="/room/image/", image_id=workSpaceRoom.id
                )
                # f"{os.getenv("LOCALHOST","http://127.0.0.1:5000/")}"+ "room/image/"+f"{workSpaceRoom.id}"
